...
export function registerWorkItemTools(server: FastMCP, workItemService: WorkItemService) {
    server.addTool({
        name: 'listWorkItems',
    ...
    server.addTool({
        ...
        parameters: z.object({ project: z.string(), type: z.string(), fields: z.record(z.any()) }),
        async execute(args) {
    ...
    server.addTool({
        ...
        async execute(args) {
            const { project } = args;
            const types: WorkItemType[] = await workItemService.listWorkItemTypes(project);
